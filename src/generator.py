...
- "Let's dive in", "In conclusion" 같은 영어식 직역 표현
- "AI", "인공지능이 작성", "생성형" 자기 정체 노출

→ 대신: "사실 핵심은…", "여기서 짚어볼 점은…", "그렇다면 왜 지금…",
  "한 발 더 들어가 보면…", "장기적으로 봤을 때…", "실무 관점에선…"
...
(A) Professional Intro — 첫 번째 <h2> 아래 첫 <p> (또는 1~2 단락):
    · 다루는 사건/이슈가 무엇인지 명확히 (사건·주체·시점)
    · 독자가 왜 지금 이 글을 읽어야 하는지 (이해관계·시의성)
    · 글을 다 읽었을 때 얻게 될 가치 (의사결정 단서 / 모니터링 지표 등)
    · "안녕하세요" / "Let's dive in" / "이 글에서는" 같은 진입 표현 금지.
      바로 사건의 핵심 또는 가장 중요한 수치에서 시작.

(B) Deep Analysis — 본문 중반의 모든 <h2> 섹션:
...
(C) Actionable Takeaways — 본문 후반에 반드시 1개 <h3> 섹션:
    헤딩은 자연스러운 한국어 (이모지/정형 템플릿 금지). 예:
      "투자자를 위한 시사점"
      "우리의 대응 전략"
      "지금 점검해야 할 체크리스트"
      "다음 분기에 살펴봐야 할 신호"
      "엔터프라이즈 도입 결정자가 따져볼 것"
    내용은 추상이 아닌 구체적·실행 가능한 권고 (<ul><li> 권장, 3~6개):
      · 어떤 종목/ETF/상품을 어떤 조건에서 검토할지
      · 어떤 지표(금리/PMI/PER/ARR 등)를 어떤 주기로 모니터링할지
      · 어떤 행동(리밸런싱/리모델링/계약갱신 등)을 언제 할지
...
---
<h2>...(Intro 섹션 헤딩)...</h2>
<p>...(Professional Intro — 사건+이해관계+가치)...</p>
<h2>...</h2>
<p>... <strong>핵심 수치/용어</strong> ...</p>
<h3>...(자연 한국어 헤딩)...</h3>
<p>...</p>
...
글 구조 (규칙 5 엄수):
  (A) Professional Intro — 첫 <h2> 직후 1~2 단락:
      사건 + 독자 이해관계 + 글을 읽고 얻을 가치 (진입 클리셰 금지)
  (B) Deep Analysis — 본문 중반 <h2> 섹션들:
      원인-결과 사슬 + 구체 수치 + 메커니즘 + 다관점 + 한계/리스크
  (C) Actionable Takeaways — 본문 후반 <h3> 섹션 (3~6개 항목 <ul>):
      어떤 종목/지표/행동을 언제·어떻게 — 추상 금지, 실행 가능 권고
  (D) Definitive Conclusion — 참고자료 직전 1~2 단락:
      "결론적으로" 금지, "장기적으로 봤을 때…" 같은 자연 표현
...
품질 체크리스트 (작성 후 자체 검증, 모두 충족해야 함):
  ...
  □ "결론적으로" / "Let's dive in" / "안녕하세요" / "이 글에서는" 등 금지 표현 0회
